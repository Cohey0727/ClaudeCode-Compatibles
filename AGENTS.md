...
- **勝手にブランチを切るな・切り替えるな。** 作業開始前に必ず「メインチェックアウトが今どのブランチにいるか」を `git -C <メインチェックアウト> branch --show-current` で確認し、**そのブランチでそのまま作業する**。`git checkout -b` / `git switch -c` / `git branch <new>` / worktree での新規ブランチ作成は、ユーザーが明示的に指示したときだけ。worktree を作る場合も「今いるブランチの worktree」を作る。worktree の中から `git branch --show-current` を見ても意味がない（自分が作ったブランチが出るだけ）。
